...
export function fixturePlanSession(overrides: Partial<ReviewSession> = {}): ReviewSession {
  return {
    ...
    artifact: { type: "plan", content: FIXTURE_PLAN, meta: { title: "Migration Plan", agent: "agent/worker-3" } },
...
export function fixtureDiffSession(overrides: Partial<ReviewSession> = {}): ReviewSession {
  return {
    ...
    id: "s_story_diff",
    workspace: { repoRoot: "/repo", branch: "main" },
    artifact: { type: "diff", content: FIXTURE_PATCH, meta: { title: "working tree", agent: "agent/worker-3" } },
