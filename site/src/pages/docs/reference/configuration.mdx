...
| `leader`                              | `ctrl+g`              | the command leader          |

The `leader` is the prefix for the thread view's structural commands: press it,
then a letter (`ctrl+g` `x` cuts the block). Letters type a comment, so the
leader keeps the commands reachable without a held modifier. See
[Keyboard & mouse](/docs/reference/keyboard/#the-command-leader).
