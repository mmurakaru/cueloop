...
next: { title: "VCS extensions", href: "/docs/reference/vcs-extensions/" }
---
...
Use `provider = "auto"` to select JJ in a colocated checkout and Git in a Git
checkout. Set `provider = "git"`, `"jj"`, or a registered adapter ID to choose
one explicitly. Extensions load from the user config only.
...
See [Review diffs](/docs/concepts/diffs/) and [VCS extensions](/docs/reference/vcs-extensions/).
