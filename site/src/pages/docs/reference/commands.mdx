...
- **`cueloop share`** uploads the plan to the gateway and copies a single
  ...
  copy and discussion.
- **`cueloop share pull`** adds a teammate's annotations to your plan. Run
...
Posts a completed Thread back to a pull request as a GitHub review without
opening the UI. Messages map directly:
...
a new Thread. See [Branches and checkpoints](/docs/concepts/history/) for the
full behavior.

**`cueloop refine`** writes a Markdown report of reviewer annotations from past
